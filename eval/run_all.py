#!/usr/bin/env python3
"""Orchestrate corpus scoring, held-out metrics, and evaluation artifacts."""
...
try:
    ...
    from .evaluation_numbers import category_averages, compute_evaluation_numbers, read_csv, read_results
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--evaluation-table", type=Path, default=Path("eval/RESULTS.md"))
    parser.add_argument("--evaluation-json", type=Path, default=Path("eval/evaluation_numbers.json"))
    parser.add_argument("--evaluation-numbers-md", type=Path, default=Path("eval/evaluation_numbers.md"))
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        write_numbers(numbers, args.evaluation_json.resolve(), args.evaluation_numbers_md.resolve())
        args.evaluation_table.resolve().parent.mkdir(parents=True, exist_ok=True)
        args.evaluation_table.resolve().write_text(
            render_results_markdown(results, metrics, numbers, args.vt_results.resolve()),
            encoding="utf-8",
        )
    except (OSError, ValueError) as exc:
        print(f"Unable to render evaluation outputs: {exc}", file=sys.stderr)
        return 2
    print_summary(numbers, results, args.vt_results.resolve())
    print(f"[table] {args.evaluation_table.resolve()}")
    print(f"[evaluation] {args.evaluation_json.resolve()}")
...
def write_numbers(numbers: Dict[str, Any], json_path: Path, markdown_path: Path) -> None:
    ...
    lines = ["# Evaluation Numbers", "", "Generated by `eval/run_all.py` from the labeled evaluation artifacts.", ""]
